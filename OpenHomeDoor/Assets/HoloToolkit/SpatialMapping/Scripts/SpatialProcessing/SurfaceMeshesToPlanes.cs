    ...
    public class SurfaceMeshesToPlanes : Singleton<SurfaceMeshesToPlanes>
    {
        ...
        private IEnumerator MakePlanesRoutine()
        {
            ...
            for (int index = 0; index < planes.Length; index++)
            {
                ...
                destinationPlane.transform.parent =  planePalent.transform;
                var surfacePlane = destinationPlane.GetComponent<SurfacePlane>();

                // 追記
                // ManagerSinglton.Instance.planeNum++;
                // if (ManagerSinglton.Instance.planeNum >= 10 && once)
                // {
                //     ManagerSinglton.Instance.doorExist.Value = true;
                //     Debug.Log("ff;");
                //     once = false;
                // }
